...
namespace MVC_Group7_demo_BLL.Services.Implementation
{
    public class OrdersServices : IOrdersServices
    {
        ...
        public async Task<(bool, string?)> AddProductOrderAsync(AddProductToOrderDTO readProduct, string customerId)
        {
            ...
            var currentOrder = customerResult.Item1.Orders.OrderByDescending(o => o.CreatedOn).FirstOrDefault(o => o.isDeleted == false && o.isFinalized == false);
        ...
        public async Task<(bool, string?)> CancelOrder(int id)
        {
            try
            {
                

                var finalizedOrder = await ordersRepo.GetByIdAsync(id);

                var diff = finalizedOrder.Item1.finalizedOn.Value - DateTime.Now;
                ...
                if (finalizedOrder.Item1 == null)
                {
                    Console.WriteLine("No id exist");
                    return (false, "No id exist");
                }

                foreach(var po in finalizedOrder.Item1.ProductOrders)
                {
                    var product = await productRepo.GetByIdAsync(po.Productid);
                    if(product.Item1 == null)
                    {
                        Console.WriteLine("problem here");
                        return (false, "problem in product");
                    }
                ...
                foreach (var po in finalizedOrder.Item1.ProductOrders)
                {
                    ...
                    var updateRes = await productRepo.UpdateStockAsync(po.Productid, po.Quantity + product.Item1.Stock, "System");
        ...
        public async Task<(List<FinalizedOrder>?,string?)> GetFinalizedOrders(string customerId)
        {
            try
            {
                ...
                {
                    Console.WriteLine("dasdadasd");
                    return (null, "List is null");
                }

                List<FinalizedOrder> ordersDTOs = new List<FinalizedOrder>();

                foreach (var o in CustomerOrders)
                {
                    ordersDTOs.Add(new FinalizedOrder()
                    {
                        OrderId = o.Order_id,
                ...
                Console.WriteLine("correct in BLL");
        ...
        public async Task<(bool, string?)> finalizeOrderAsync(string customerId, string paymentMethod)
        {
            try
            {
                ...
                if(res.Item1 == false)
                {
                    ...
                    return (false, "Error came from Repo");
                }
