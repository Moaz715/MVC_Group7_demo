...
namespace MVC_Group7_demo_PLL.Controllers
{
    ...
    public class OrderController : Controller
    {
        ...
        public async Task<IActionResult> ViewFinalizedOrders()
        {
            ...
            var res = await ordersServices.GetFinalizedOrders(user.Id);
            if(res.Item1 == null)
            {
                Console.WriteLine("obaaaaa");
            }

            return View(res.Item1);
        }

        public async Task<IActionResult> FinalizeOrder(string paymentMethod)
        {
            var user = await userManager.GetUserAsync(User);
            if (user == null)
            {
            ...
            var res = await ordersServices.finalizeOrderAsync(user.Id,paymentMethod);

            if (res.Item1)
            {
                TempData["FinalizeOrderSuccess"] = "Your order has been finalized successfully!";
                return RedirectToAction("Index","MainMenu");
            }
            else
            {
                TempData["FinalizeOrderError"] = res.Item2 ?? "Failed to finalize order.";
        ...
        {
            var res = await ordersServices.CancelOrder(id);

            if (res.Item1)
            {
                return RedirectToAction("ViewFinalizedOrders", "Order");
            }
