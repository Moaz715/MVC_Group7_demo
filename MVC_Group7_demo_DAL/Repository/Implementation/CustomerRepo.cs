...
namespace MVC_Group7_demo_DAL.Repository.Implementation
{
    public class CustomerRepo : ICustomerRepo
    {
        ...
        public async Task<(bool, string?)> Create(Customer customer)
        {
        ...
        public async Task<(Customer?, string?)> GetById(string id)
        {
            try
            {
                var res = await db.Customers.Include(c => c.Orders).ThenInclude(po => po.ProductOrders).ThenInclude(p => p.Product).Where(c => c.UserId == id).FirstOrDefaultAsync();
